...
def has_ve(layer_idx, n_layer):
    return False  # disable VE to reduce overfitting on small corpus
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        mlp_dim = 8 * config.n_embd // 3  # iso-param to 4x relu^2
        self.c_gate = nn.Linear(config.n_embd, mlp_dim, bias=False)
...
# Optimization
TOTAL_BATCH_SIZE = 2**16 # ~65K tokens per optimizer step (2x more steps vs 2^17)
